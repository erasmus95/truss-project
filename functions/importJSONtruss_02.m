function [C,Sx,Sy,X,Y,L] = importJSONtruss_02()
    cd("json_files\");
    ...
    span_start = [min(nodes(:,1)),1]; % start at the smallest x value and 
    for i = 1:height(nodes)
        if nodes(i,1) >= span_start(1) && nodes(i,2)==0 && nodes(i,1) ~= 34
            span_start = [nodes(i,1),i];
        end
    end
    Sy(span_start,2)=1;
    Sx(span_start,1)=1;
